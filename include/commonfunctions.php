...
function checkTableName($shortTName )
{
	...
	if ("I_ConvocatoriasNacionales" == $shortTName )
		return true;
	if ("List_ConvocatoriasNacionales" == $shortTName )
		return true;
	if ("I_ConvNacMailsNotif" == $shortTName )
		return true;
	if ("Mails_para_notificar" == $shortTName )
		return true;
	if ("ConvocatoriasNacionales_PrintPDF" == $shortTName )
		return true;
	if ("I_ConvocatoriasProvinciales" == $shortTName )
		return true;
	if ("List_ConvocatoriasProvinciales" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("I_ConvocatoriasNacionales");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="I_ConvocatoriasNacionales";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("List_ConvocatoriasNacionales");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="List_ConvocatoriasNacionales";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("I_ConvNacMailsNotif");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="I_ConvNacMailsNotif";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Mails para notificar");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Mails para notificar";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("ConvocatoriasNacionales_PrintPDF");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="ConvocatoriasNacionales_PrintPDF";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("I_ConvocatoriasProvinciales");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="I_ConvocatoriasProvinciales";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("List_ConvocatoriasProvinciales");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="List_ConvocatoriasProvinciales";
	}
	return $arr;
}
...
function GetUserPermissionsStatic( $table )
{
	...
	$extraPerm = "";
	if( $table=="I_ConvocatoriasNacionales" )
	{
		if( $sUserGroup=="<Guest>" )
		{
						return "A".$extraPerm;
...
//	default permissions
	...
	if( $table=="List_ConvocatoriasNacionales" )
	{
		if( $sUserGroup=="<Guest>" )
		{
						return "".$extraPerm;
...
//	default permissions
	...
	if( $table=="ConvocatoriasNacionales_PrintPDF" )
	{
		if( $sUserGroup=="<Guest>" )
		{
						return "".$extraPerm;
...
//	default permissions
	...
	if( $table=="I_ConvocatoriasProvinciales" )
	{
		if( $sUserGroup=="<Guest>" )
		{
						return "A".$extraPerm;
...
//	default permissions
	...
	if( $table=="List_ConvocatoriasProvinciales" )
	{
		if( $sUserGroup=="<Guest>" )
		{
						return "".$extraPerm;
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?38595")."\"></script>";


	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?38595")."\"></script>";
